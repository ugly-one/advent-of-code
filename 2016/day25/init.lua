...
for i = 1, #lines do
  ...
  local arg1 = string.match(lines[i], 'out (%S+)')
  if arg1 then
    table.insert(instructions, { type = 'out', arg1 = arg1 })
  end
end
...
local function execute(a, limit)
  local counter = 1
  local signal = ""
  local reg = {}
  reg['a'] = a
  ...
  while count > 0 do
    ...
    if multiply then
      ...
      if ins.type == 'out' then
        if counter > limit then
          return signal
        end
        counter = counter + 1
        if tonumber(ins.arg1) then signal = signal .. ins.arg1 else signal = signal .. reg[ins.arg1] end
        position = position + 1
      end
...
local signal_width = 30
for i = 1, 1000 do
  local signal = execute(i, signal_width)
  local array = lib.to_table(signal)
  local wrong = false
  for j = 2, signal_width do
